...
ConditionVariable::ConditionVariable(Lock& user_lock, const char* aName)
    : ConditionVariable(user_lock) {
  user_lock.mName = aName;
}
...
mozilla::CVStatus2 ConditionVariable::TimedWait(
    const mozilla::TimeDuration& rel_time) {
  int64_t usecs = rel_time.ToMicroseconds();
...
#  endif  // OS_ANDROID && HAVE_PTHREAD_COND_TIMEDWAIT_MONOTONIC
  ...
  return rv == ETIMEDOUT ? mozilla::CVStatus2::Timeout
                         : mozilla::CVStatus2::NoTimeout;
}
