...
def CheckedEval(file_contents):
  ...
  try:
    parsed = ast.parse(file_contents, mode='eval')
  except SyntaxError:
    # Re-raise so callers get the usual SyntaxError info.
    raise
  return CheckNode(parsed.body, [])
...
def CheckNode(node, keypath):
  if isinstance(node, ast.Dict):
    ...
    for key_node, value_node in zip(node.keys, node.values):
      key = CheckNode(key_node, keypath)
      if not isinstance(key, str):
        raise GypError("Key " + repr(key) + " is not a string at level " +
      ...
      kp = list(keypath)
      kp.append(key)
      dict_value[key] = CheckNode(value_node, kp)
    return dict_value
    ...
    operand = CheckNode(node.operand, keypath)
    if isinstance(operand, (int, float, complex)):
      return +operand if isinstance(node.op, ast.UAdd) else -operand
  elif isinstance(node, ast.BinOp) and isinstance(node.op, ast.Add):
    left = CheckNode(node.left, keypath)
    right = CheckNode(node.right, keypath)
    return left + right
  else:
    try:
